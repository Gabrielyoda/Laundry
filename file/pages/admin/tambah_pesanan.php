						...
						<div class="col-md-12">
							...
							<div class="panel">
								<div class="panel-heading">
									<h3 class="panel-title">Input Data Pesanan</h3>
                                </div>
                                <form action="tambah_pesanan_proses.php" method="post" onsubmit="return validasi(this)">
								<div class="panel-body">
									<table class="table">
										<tbody>
                                        <tr>
                                        <td>Nama Pelanggan <br><?php
                                        $query_user = "select*from user where level='2'";
                                        $result_user = mysqli_query($connection,$query_user);                                        
                                        echo '<select class="form-control" name="id_user">';
										while($rec = mysqli_fetch_array($result_user)){
											echo '<option value='.$rec['id_user'].'>'.$rec['nama_user'].'</option>';
                                        }
                                        echo "</select>";
										?>
    								...
    								<tr> 
  									...
  									<tbody>
										<?php
											$sql = "select nama_barang,harga_laundry from barang ORDER BY id_barang";
											$tampilkan = mysqli_query($connection,$sql); 
											while($select_result = mysqli_fetch_array($tampilkan))
											  {
											  $nama_barang	 	= $select_result['nama_barang'] ;
											  $harga_laundry	 = $select_result['harga_laundry'] ;
											  echo"
											  <tr>  		
												<td>$nama_barang<input type='hidden' class='form-control' value='$nama_barang'name='nama_barang[]' ></td>
												<td>$harga_laundry<input type='hidden' class='form-control' value='$harga_laundry'name='harga_laundry[]' ></td>
